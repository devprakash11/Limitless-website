...
import { listRows, updateRow, insertRow } from "../services/database.js";
import { supabaseAdmin } from "../config/supabase.js";
import { ok, created } from "../utils/api.js";

const text = (max) => z.string().trim().min(1).max(max);
const invoiceSchema = z.object({ client_id: z.string().uuid(), invoice_number: text(80), amount: z.number().nonnegative(), status: z.enum(["DRAFT","SENT","PAID","OVERDUE","CANCELLED"]).default("DRAFT"), due_date: z.string().date().optional().nullable(), description: text(10000).optional().nullable() });
const paymentSchema = z.object({ invoice_id: z.string().uuid(), client_id: z.string().uuid(), amount: z.number().positive(), method: text(80), reference: text(160).optional().nullable(), status: z.enum(["PENDING","SUCCESS","FAILED","REFUNDED"]).default("SUCCESS") });
